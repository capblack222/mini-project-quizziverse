...
io.on('connection', (socket) => {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('kahootGames').find(query).toArray(function(err, result){
                ...
                if(result[0] !== undefined){
                    ...
                    //Creates a quiz with pin and host id
    ...
    socket.on('host-join-game', (data) => {
        ...
        //Gets quiz with old host id
        var game = games.getGame(oldHostId);
        if(game){
            //Changes the quiz host id to new host id
    ...
    socket.on('player-join', (params) => {
        ...
        // a quiz found or not with pin provided by player
        var gameFound = false; 
        
        // For each quiz in the Games class
        ...
        if(gameFound == false){
            // Player is sent back to 'join' page because quiz was not found with pin
            socket.emit('noGameFound'); 
        }
